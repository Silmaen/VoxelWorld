...
namespace owl::math
{
	...
	public:
		 ...
		 */
		constexpr FrameSize(const uint32_t iWidth, const uint32_t iHeight) : m_width{iWidth}, m_height{iHeight}
		{
		}
		...
		/**
		 * @brief Direct const access to the width.
		 * @return The width
		 */
		[[nodiscard]] const uint32_t& width() const { return m_width; }

		/**
		 * @brief Direct const access to the height.
		 * @return The width
		 */
		[[nodiscard]] const uint32_t& height() const { return m_height; }
		...
		/**
		 * @brief Compute the size ratio.
		 * @return The size ratio.
		 */
		[[nodiscard]] float ratio() const { return static_cast<float>(m_width) / static_cast<float>(m_height); }
