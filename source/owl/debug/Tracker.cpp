...
void Tracker::allocate(size_t size) {
  ...
  globalAllocationState.memoryPeek =
      std::max(
...
void Tracker::deallocate(size_t size) {
  ...
  currentAllocationState.allocatedMemory =
      currentAllocationState.allocatedMemory > size? currentAllocationState.allocatedMemory -size:0;
  ++globalAllocationState.deallocationCalls;
  globalAllocationState.allocatedMemory =
      globalAllocationState.allocatedMemory > size? globalAllocationState.allocatedMemory -size:0;
}
