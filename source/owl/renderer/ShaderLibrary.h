...
public:
	 ...
	 */
	void add(const shrd<Shader> &shader);
	 ...
	 */
	void addNRename(const std::string &name, const shrd<Shader> &shader);
	 ...
	 */
	shrd<Shader> load(const std::filesystem::path &file);
	 ...
	 */
	shrd<Shader> get(const std::string &name);
...
private:
	/// List of shaders
	std::unordered_map<std::string, shrd<Shader>> shaders;
};
#ifdef __clang__
