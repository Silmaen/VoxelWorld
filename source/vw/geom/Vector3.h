 ...
 * @brief Class Vector3
 */
template <class DataType> class Vector3 {
public:
  /**
  ...
  explicit Vector3(const Vector3<DatatypeOther> &other)
   ...
   */
  template <class DatatypeOther>
  explicit Vector3(const Vector2<DatatypeOther> &other)
      : X{static_cast<DataType>(other[0])}, Y{static_cast<DataType>(other[1])},
        Z{0} {}
  /**
   ...
   * @return data
   */
  const DataType &operator[](uint8_t idx) const {
   ...
   * @return data
   */
  const DataType &z() const { return Z; }
   ...
   * @return This actualized vector
   */
  Vector3 &operator*=(const DataType &other) {
    X *= other;
    Y *= other;
    Z *= other;
    return *this;
   ...
   * @return Result vector
   */
  Vector3 operator*(const DataType &other) const {
    Vector3 res{*this};
    res *= other;
    return res;
  }
   ...
   * @return Result vector
   */
  friend Vector3 operator*(const DataType &other, const Vector3 &vec) {
    return vec * other;
  }
  /**
   * @brief Division operator
   ...
   */
  DataType dot(const Vector3 &other) const {
    return X * other.X + Y * other.Y + Z * other.Z;
  }
  /**
...
inline void to_json(nlohmann::json &jso, const Vector3<DataType> &vect) {
  jso = nlohmann::json{vect.x(), vect.y(), vect.z()};
}
/**
