...
struct VideoCapture {
	...
	void fillFrame(owl::shared<owl::renderer::Texture>) {
	}
...
void CameraSystem::setCamera(int32_t id) {
	auto &settings = IO::DroneSettings::get();
	const int32_t last = settings.cameraId;
...
CameraSystem::CameraInfo CameraSystem::getCurrentCamera() const {
	const auto res = std::find_if(cameraList.begin(), cameraList.end(), [](const CameraInfo &cam) { return cam.id == IO::DroneSettings::get().cameraId; });
